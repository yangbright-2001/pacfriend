...
class GraphSearchTest(testClasses.TestCase):
    ...
    def getSolInfo(self, search):
        ...
        # print("expanded states is:", problem.getExpandedStates())
...
class CornerHeuristicSanity(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for state in states:
            ...
            heuristics.append((state[0],searchAgents.cornersHeuristic(state, problem)))
        print("heuristics are", heuristics)
        for i in range(0, len(heuristics) - 1):
            h0 = heuristics[i][1]
            h1 = heuristics[i+1][1]
            # cornerConsistencyB
            if h0 - h1 > 1:               
                print("DEBUG2:h0 point is",heuristics[i][0], "h0 heuristics is", h0,"h1 point is",heuristics[i+1][0], "h1 heuristics is", h1)
                grades.addMessage('FAIL: inconsistent heuristic')
                return False
            # cornerPosH
            if h0 < 0 or h1 <0:
                ...
                return False
        # cornerGoalH
        if heuristics[len(heuristics) - 1][1] != 0:
