...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for d in range(0, X_RANGE + Y_RANGE + MAX_NOISE - 1):
        variableDomainsDict[OBS0].append(d)
        variableDomainsDict[OBS1].append(d)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        new_CPT = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for i in eliminationOrder:
            new_CPT, new_factor = joinFactorsByVariable(new_CPT, i)
            if len(new_factor.unconditionedVariables()) == 1:
                continue
            new_factor = eliminate(new_factor, i)
            new_CPT.append(new_factor)
        return normalize(joinFactors(new_CPT))
        "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        sum_of_value = self.total()
        if sum_of_value != 0:
            for key in self.keys():
                self[key] = self[key] / sum_of_value
        elif sum_of_value == 0: #do nothing
            return
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        value_list = list(self.keys())
        weight_list = list(self.values())
        return random.choices(value_list, weights = weight_list, k = 1)[0]
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for j in all_pos:
            posterior = self.getObservationProb(observation, pacman_pos, j, jail_pos)
            updated_belief = posterior * self.beliefs[j]
            self.beliefs[j] = updated_belief
        "*** END YOUR CODE HERE ***"
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        newposition_distribution_lst = []
        nextposition_distribution_lst = []
        all_pos = self.allPositions
        for pos in all_pos:
            newposition_distribution = self.getPositionDistribution(gameState, pos)
            newposition_distribution_lst.append([pos, newposition_distribution])
        for next_pos in all_pos:
            probability = 0
            for pos in all_pos:
                #等效于dict[pos],element[1]就是dict[pos]
                for element in newposition_distribution_lst:
                    if element[0] == pos:
                        probability += element[1][next_pos] * self.beliefs[pos]
            nextposition_distribution_lst.append([next_pos, probability])

        for next_pos in all_pos:
            for element in nextposition_distribution_lst:
                if element[0] == next_pos:  #等效于dict[next_pos],element[1]就是dict[next_pos]
                    new_belief = element[1]
            self.beliefs[next_pos] = new_belief
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        total_sample = self.numParticles
        legal_pos = self.legalPositions
        for sample_index in range(total_sample):         #ensure even distribution 
            sample_pos = self.legalPositions[sample_index % len(legal_pos)]
            self.particles.append(sample_pos)
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        #the weight of particle is the probability of observation given location and particle location
        particle_list = self.particles
        for particle_pos in particle_list:
            weight_distribution[particle_pos] += self.getObservationProb(observation, gameState.getPacmanPosition(), particle_pos, self.getJailPosition())
        sum_weight = 0
        for key in list(weight_distribution.keys()):
            sum_weight += weight_distribution[key]
        #special conditon: initialize if weight sum to 0
        if sum_weight == 0:
            self.initializeUniformly(gameState)
        else:
    ...
    def elapseTime(self, gameState):
        ...
        for index in range(len(self.particles)):
            #在当前particle位置，获取幽灵在新位置上的分布
            new_pos_distribution = self.getPositionDistribution(gameState, self.particles[index])
            #用新采样数据代替原始样本
            self.particles[index] = new_pos_distribution.sample()
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
